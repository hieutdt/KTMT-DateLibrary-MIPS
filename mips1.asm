...
runMenu5:
addi	$v0, $0, 4
la	$a0, twoRecentLeapYear
syscall

jal	TwoRecentLeapYear
j	exit
...
#6. Hai nam nhuan gan nhat voi TIME
TwoRecentLeapYear:
	subi	$sp, $sp, 8
	sw	$ra, 0($sp)
	sw	$s2, 4($sp)
	...
	addi	$s2, $s2, 1	#year += 1
	addi	$t5, $0, 0	#dem = 0
	addi	$t6, $0, 1	#hang so 1
	addi 	$t7, $0, 2	#hang so 2
Loop:
	beq 	$t5, $t7, exitRecentLeapYear
	addi	$a1, $0, 0
	jal 	checkLeapYear
	beq	$a1, $t6, printYear	#neu nhu la nam Nhuan -> in ra, tang dem len 1
	addi 	$s2, $s2, 1	#year += 1
	j 	Loop
...
#7. Kiem tra NGAY/THANG/NAM nhap vao co hop le hay khong
checkDate:
	subi	$sp, $sp, 4
	sw	$ra, 0($sp)
	
	slt 	$t4, $s2, $0
	bne	$t4, $0, False	#s2 = YEAR < 0 is TRUE
	
	slti	$t4, $s1, 0	
	bne	$t4, $0, False	#s1 = MONTH < 0 is TRUE
	
	addi	$t5, $0, 12	#temp 12
	slt	$t4, $t5, $s1 	#12 < s1 -> s1 > 12
	bne	$t4, $0, False	#s1 = MONTH > 12 is TRUE
	
	addi	$t5, $0, 1	#temp = 1
	beq	$s1, $t5, checkDay31
	addi	$t5, $0, 3	#temp 
	beq	$s1, $t5, checkDay31
	addi	$t5, $0, 5
	beq	$s1, $t5, checkDay31
...
checkMonth2:
	addi	$a1, $0, 0
	addi 	$t8, $0, 1	#hang so 1
	jal	checkLeapYear
	
	beq	$a1, $t8, checkDay29
	slti	$t6, $s0, 0
	bne	$t6, $0, False
	addi	$t7, $0, 28
	slt	$t6, $t7, $s0
...
convertToCharArray:
	...
	#load date and process
	add   $t1, $0, $s0
	addi  $t2, $0, 10 
	div   $t1, $t2
	mfhi  $t4 # last digit to $t4
	mflo  $t3 # first digit to $t3
	...
	#load year and process
	add   $t1, $0, $s2
	addi  $t2, $0, 10 
	div   $t1, $t2
	mfhi  $t6 # last digit to $t6
	mflo  $t3 # quotient to $t3
	div   $t3,$t2
	mfhi  $t5 # near last digit to $t5
	mflo  $t3 # quotient to $t3	
	div   $t3,$t2
	mfhi  $t4 # near first digit to $t4	
	mflo  $t3 # quotient to $t3
	div   $t3,$t2
	mfhi  $t3 # first digit to $t6	
	mflo  $t2 # quotient to $t3
...
twoRecentLeapYear: .asciiz "Hai nam nhuan gan nhat la:\n"

valid:	.asciiz "== Ngay thang nam hop le ==\n"
invalid: .asciiz "== Ngay thang nam khong hop le ==\n"
